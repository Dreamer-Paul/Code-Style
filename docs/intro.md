...
编写一个前端项目，技术和框架的选型是比较主要的，合适的选型可以减少实际开发产生的问题，以及改善
...
这些资源特别容易残留在项目内，如果组件设计的初期没考虑到良好的扩展性，样式未 [使用 Less 嵌套](/less/code-spec.html#选择器命名) 和 CSS Modules 的模式编写，想要找到删改涉及到的具体样式信息就特别艰难。

Tailwind 采用全局的简短类名形式，再结合 TreeShaking 过滤未使用项，就能良好的解决这样的问题。但我也不是在这里单推 Tailwind 这种 CSS 框架，编写伪元素等复杂的选择器实现，你依旧还是得编写原生 CSS 不是么。
